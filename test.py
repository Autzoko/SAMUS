...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    if opt.mode == "train":
        ...
        print("\n" + "="*60)
        print("  Dataset: %s | Model: %s" % (args.task, args.modelname))
        print("  Checkpoint: %s" % opt.load_path)
        print("="*60)
        print("  %-20s %10s %10s" % ("Metric", "Mean", "Std"))
        print("  " + "-"*42)
        print("  %-20s %9.2f%% %9.2f%%" % ("Dice", mean_dice[1], std_dice[1]))
        print("  %-20s %10.2f %10.2f" % ("Hausdorff Dist (px)", mean_hdis[1], std_hdis[1]))
        print("  %-20s %9.2f%% %9.2f%%" % ("IoU", mean_iou[1], std_iou[1]))
        print("  %-20s %9.2f%% %9.2f%%" % ("Accuracy", mean_acc[1], std_acc[1]))
        print("  %-20s %9.2f%% %9.2f%%" % ("Sensitivity", mean_se[1], std_se[1]))
        print("  %-20s %9.2f%% %9.2f%%" % ("Specificity", mean_sp[1], std_sp[1]))
        print("="*60 + "\n")
